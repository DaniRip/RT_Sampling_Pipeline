function [centr, newVoxelIndices, newVoxelCoords, clusters, runtime] = kmeans_Dij(coordinates, d_mat, k)
...
%             nonNormedDist = sum((d_mat(ii,:) - centr).^2,2);%sqrt(sum((d_mat(ii,:) - centr).^2, 2));
            diffs = (d_mat(ii,:) - centr);
            curMin = Inf;
            currLoc = clusters(ii);
            for kk = 1:k
                curDiff=diffs(kk,diffs(kk,:)~=0);
                if ~isempty(curDiff)
                    nonNormedDist = sum(curDiff.^2);
                else
                    nonNormedDist = 0;
                end
                if nonNormedDist<curMin
                    curMin = nonNormedDist;
                    currLoc = kk;
                end
            end    
            if clusters(ii)~=currLoc
